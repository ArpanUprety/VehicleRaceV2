...
public class VehicleRace {
    ...
    public static void raceCars(Vehicle car1, Vehicle car2) {
        ...
        //call the method checkForKart(car1, speed2, car2)

        // Ensure speeds are positive
        speed1 = Math.max(speed1, 0);
        speed2 = Math.max(speed2, 0);

        speed1 = checkForCart(car2, speed1, car1);
        speed2 = checkForCart(car1, speed2, car2);
    ...
    public static int checkForCart(Vehicle c1, int speed, Vehicle c2){
        if (c1 instanceof Kart){
            SpecialItem a = ((Kart) c1).detailedAttack();
            String color = a.getTextColor();

            int ogspeed = speed;
            speed -= a.getDelay();
            System.out.println( color+ c1.getBrand() + " slowed " + c2.getBrand() + " with a " + a.getItem() + " reducing the speed of "
            + ogspeed + "mph to " + speed + "mph" + ConsoleColors.RESET);
